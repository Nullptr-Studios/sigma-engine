...
void Sigma::AudioEngine::Set3DListenerPosition(float posX, float posY, float posZ, float forwardX, float forwardY,
...
void Sigma::AudioEngine::LoadEvent(
    const char *eventName, const std::vector<std::pair<const char *, float>>
...
void Sigma::AudioEngine::Set3DChannelPosition(const AudioData &audioData, FMOD::Channel *channel) const {
  FMOD_VECTOR position = {audioData.GetPosition().x * DISTANCEFACTOR, audioData.GetPosition().y * DISTANCEFACTOR,
...
void Sigma::AudioEngine::DebugEventInfo(const FMOD::Studio::EventDescription *eventDescription) {
  ...
  std::cout << "FMOD EventDescription has " << params << " parameter descriptions, " << (is3D ? " is " : " isn't ")
            << " 3D, " << (isOneshot ? " is " : " isn't ") << " oneshot, "
